...
PYBIND11_MODULE(PyAnsatzEvolve, m) {
    ...
    py::class_<stateAnsatzManager>(m, "stateAnsatzManager")
        ...
        .def(
            "storeOperators",
            [](stateAnsatzManager &self, const std::vector<std::vector<int>> &operators, bool isHermitian)
            {
                ...
                {
                    if (op.size() != 4) throw std::length_error("Operator must be 4 long");
                    for (int8_t i = 0; i < 4; i++)
                    {
                        if (op[i] > std::numeric_limits<int8_t>::max()) throw std::out_of_range("Operator index too big, < 127");
                        e[i] = op[i] - 1;
                    }
                    if (isHermitian) e.isHermitian = true;
                    excs.push_back(e);
                }
                self.storeOperators(excs);
            },
            py::arg("Operators"),
            py::arg("isHermitian") = false)
        ...
        .def("getFinalState",
             ...
             {
                 vector<numType> ret;
                 self.getFinalState(ret);
                 return static_cast<vector<numType>::EigenVector>(ret);
             })
        .def("getGradient",
             [](stateAnsatzManager &self)
             {
                 vector<realNumType> ret;
                 self.getGradient(ret);
                 return static_cast<vector<realNumType>::EigenVector>(ret);
             })
        .def("getGradientComp",
             [](stateAnsatzManager &self)
             {
                 vector<realNumType> ret;
                 self.getGradientComp(ret);
                 return static_cast<vector<realNumType>::EigenVector>(ret);
             })
        .def("getHessian",
             [](stateAnsatzManager &self)
             {
                 Matrix<realNumType>::EigenMatrix ret;
                 self.getHessian(ret);
                 return ret;
             })
        .def("getHessianComp",
        ...
        .def("getExpectationValue",
             ...
             {
                 realNumType ret;
                 self.getExpectationValue(angles, ret);
                 return ret;
             })
        .def("getFinalState",
             [](stateAnsatzManager &self, const std::vector<realNumType> &angles)
        ...
        .def("getGradientComp",
             ...
             {
                 vector<realNumType> ret;
                 self.getGradientComp(angles, ret);
                 return static_cast<vector<realNumType>::EigenVector>(ret);
             })
        .def("getHessian",
             [](stateAnsatzManager &self, const std::vector<realNumType> &angles)
        ...
        .def("getHessianComp",
             ...
             {
                 Matrix<realNumType>::EigenMatrix ret;
                 self.getHessianComp(angles, ret);
                 return ret;
             })
    ...
    m.def("loadParameters",
          [](const std::string &orderFilePath, const std::string &parameterFilepath, std::vector<ansatz::rotationElement> &templatePath)
          {
