...
if __name__ == "__main__":
    ...
    for Index,RawAngles in enumerate(ParameterList):
        ...
        print(RawAngles)
        man.setAngles(RawAngles)
        EnergyStart = man.getExpectationValue()
        print(f"Energy At Start: {EnergyStart}")
        man.optimise()
        EnergyEnd = man.getExpectationValue()
        print(f"Energy At End: {EnergyEnd}")
        
        #Set to zero
        Angles = man.getUncompressedAngles()
        state = man.getFinalState()
        if state[InitialStateIndices[0]] < 0:
            Angles = CanonicaliseAngles(man,Angles,Operators,SDSPositions,InitialStateCoeffs,InitialStateIndices,invertSignOfWavefunction=True)
        else:
            Angles = CanonicaliseAngles(man,Angles,Operators,SDSPositions,InitialStateCoeffs,InitialStateIndices,invertSignOfWavefunction=False)
        man.setAngles(Angles)
        EnergyEnd = man.getExpectationValue()
        print(f"Energy After Canonical: {EnergyEnd}")
        if not np.isclose(EnergyStart,EnergyEnd,1e-9):
            print(f"Minima not optimised correctly. Before:{EnergyStart}, End:{EnergyEnd} Index:{Index}")
            quit()
        CompAngles = man.getAngles()
        print(CompAngles)
        with open(PathBase + "canonicalmin","a") as f:
            for i,ang in enumerate(CompAngles):
                f.write(f"{ang:.16f}\n")
